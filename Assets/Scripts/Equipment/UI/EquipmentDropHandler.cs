    ...
    IDropHandler
{
    ...
    public void OnDrop(PointerEventData eventData)
    {
        ...
        {
            Debug.LogWarning(
                "EquipmentDropHandler could not find EquipmentManager.",
                gameObject
            );
        ...
        EquipmentInstance draggedEquipment =
            InventoryDragHandler.DraggedEquipment;

        if (draggedEquipment == null ||
            draggedEquipment.equipmentData == null)
        {
            Debug.Log(
                "No valid inventory equipment was dropped."
            );
    ...
    private void TryEquipDroppedItem(
        EquipmentInstance equipment
    )
    {
        EquipmentUISlotType slotType =
            _equipmentSlotUI.SlotType;

        int slotIndex =
            _equipmentSlotUI.SlotIndex;

        if (slotType == EquipmentUISlotType.Weapon)
        {
            EquipmentManager.Instance.EquipWeaponToSlot(
                equipment,
                slotIndex
            );
        ...
        {
            EquipmentManager.Instance.EquipArtifactToSlot(
                equipment,
                slotIndex
            );
        }
